...
public class ProtocolTest {
	// public static String testData = "Hello world!! Nice to see you, hope that this are more
	// than 32 bytes!!!";
	/*public static String testData = "Wie oben erwaehnt, fuehrt eine genauere, wissenschaftliche Betrachtung zu komplexeren Definitions- und Beschreibungsversuchen. Die Eigenschaft des „Text-Seins“ bezeichnet man als Textualitaet, die sprachwissenschaftliche Untersuchung von Texten ist die Textlinguistik. Diese Disziplin stellt verschiedene Textualitaetskriterien zur Verfuegung.\n"
			+ "\n"
			+ "Robert-Alain de Beaugrande und Wolfgang Ulrich Dressler stellten 1981 eine Reihe solcher Kriterien vor. Diese Kriterien beziehen sich einerseits auf die Merkmale des Textes selbst (Kohaesion, also formaler Zusammenhalt und Kohaerenz, also logischer Zusammenhalt), andererseits auf die Merkmale einer Kommunikations­situation, aus der der betreffende Text entsteht bzw. in der er eingesetzt wird (Intentionalitaet, Akzeptabilitaet, Informativitaet, Situationalitaet).\n"
			+ "\n"
			+ "Kohaesion und Kohaerenz gehoeren zu den am weitesten akzeptierten Textualitaetskriterien, aber auch hier gibt es Abweichungen: Es gibt durchaus Texte, welche aus zusammenhanglosen Worten oder gar Lauten, zum Teil auch aus bis zu blossen Geraeuschen reduzierten Klangmalereien bestehen, und die, im Ganzen dennoch vielschichtig interpretierbar, eine eigene Art von Textualitaet erreichen (zum Beispiel Dada-Gedichte).\n"
			+ "\n"
			+ "Hier kommen die situationsbezogenen Textualitaetskriterien ins Spiel: Texte sind auch dadurch bestimmt, dass ein Sender sie mit einer bestimmten Absicht (Intention) produziert und/oder ein Empfaenger sie als solche akzeptiert. Ob ein Text fuer einen bestimmten Empfaenger akzeptabel ist, haengt wiederum stark davon ab, ob dieser einen Zusammenhang der empfangenen aeusserung mit seiner Situation herstellen, den Text also in seine Vorstellungswelt „einbauen“ kann (Situationalitaet), und ob der Text fuer ihn informativ ist, also in einem bestimmten Verhaeltnis erwartete und unerwartete, bekannte und neue Elemente enthaelt. Um auf das Beispiel des Dada-Gedichtes zurueckzukommen: Ein nicht offensichtlich kohaesiver oder kohaerenter Text kann als solcher akzeptabel sein, wenn der Empfaenger davon ausgeht, dass die Intention des Senders ein hohes Mass an ueberraschenden oder von der Norm abweichenden Elementen im Text erfordert.\n"
			+ "\n"
			+ "Die Intertextualitaet als letztes der Textualitaetskriterien nach de Beaugrande und Dressler ist die Eigenschaft eines Textes, mit anderen Texten in Verbindung zu stehen und auf sie Bezug zu nehmen. In literarischen Texten geschieht dies haeufig durch bewusste Verweise und Zitate, Intertextualitaet kann ihren Ausdruck jedoch z. B. auch darin finden, dass ein Gebrauchstext die ueblichen Konventionen seiner Textsorte erfuellt.\n"
			+ "\n"
			+ "Die einzelnen hier angefuehrten Textualitaetskriterien sind in ihrer Interpretation durch de Beaugrande/Dressler zum Teil umstritten. Allgemein anerkannt ist, dass ein Text eine erkennbare kommunikative Funktion hat, die durch die kommunikative Absicht des Senders und die Erwartungen des Empfaengers bestimmt wird, dass er als aeusserung abgegrenzt und thematisch orientiert ist, d. h. ueber einen inhaltlichen Kern verfuegt. Eine solche Textdefinition aus kommunikativ-pragmatischer Perspektive bietet Susanne Goepferich:";
*/
	public static String testData = "Miusov, as a man man of breeding and deilcacy, could not but feel some inwrd qualms, when he reached the Father Superior's with Ivan: he felt ashamed of havin lost his temper. He felt that he ought to have disdaimed that despicable wretch, Fyodor Pavlovitch, too much to have been upset by him in Father Zossima's cell, and so to have forgotten himself. \"Teh monks were not to blame, in any case,\" he reflceted, on the steps. \"And if they're decent people here (and the Father Superior, I understand, is a nobleman) why not be friendly and courteous withthem? I won't argue, I'll fall in with everything, I'll win them by politness, and show them that I've nothing to do with that Aesop, thta buffoon, that Pierrot, and have merely been takken in over this affair, just as they have.\"\r\n" + 
			"\r\n" + 
	...
	public static void main(String[] args) {
		...
		SerialDriver comm = SerialDriver.getInstance(32, "ProtocolTest");
		int i = 0;
		...
		// UART_EIVE_Protocol_Send sender = new UART_EIVE_Protocol_Send(comm);

		// UART_EIVE_Protocol_Recv receiver = new UART_EIVE_Protocol_Recv(comm);
	...
	private static void receive_data(SerialDriver comm) {
		System.out.println("----------------------RECEIVING TEST----------------------");
		comm.connect("COM99");
		UART_EIVE_Protocol_Recv receiver = new UART_EIVE_Protocol_Recv(comm);

		while (receiver.UART_Recv_Data() != Constants.XST_SUCCESS)
			;

		System.out.println("Done!");
	}

	private static void send_data(SerialDriver comm) {
		System.out.println("----------------------SENDING TEST----------------------");

		comm.connect("COM100");
		UART_EIVE_Protocol_Send sender = new UART_EIVE_Protocol_Send(comm);

		int status = sender.UART_Send_Data(Constants.UART_TC, testData.getBytes(), testData.length());
